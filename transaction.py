...
class transaction:
	...
	category = 0
	amount = 0
	def __init__(self, transaction_fields):
		for field in transaction_fields:
			if field[0] == "D":
				date = parser.parse(field[1:], fuzzy=True) #parsing dates can be hard
			elif field[0] == "T":
				amount = int(field[1:].replace(".", "")) #store amount as int to prevent floating point err
			else:
				pass
		address = 0
		cleared = 0
